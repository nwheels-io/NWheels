...
namespace NWheels.Stacks.EntityFramework.Tests.Integration
{
    ...
    public static class ThisTestCrudOperations
    {
        public static class Repository1
        {
            ...
            private static void InsertOrder1(Interfaces.Repository1.IOnlineStoreRepository repo)
            {
                using ( repo )
                {
                    ...
                    var customerRepo =
                        ((EfEntityRepository<IR1.ICustomer, HardCodedImplementations.EfEntityObject_Customer, HardCodedImplementations.EfEntityObject_Customer>)
                            repo.Customers);

                    var customer = repo.Customers.Single(c => c.FullName == "John Smith");//.As<IPersistableObject>();

                    order.Customer = customer;//repo.Customers.First(c => c.FullName == "John Smith");
                    //customerRepo.ObjectSet.Attach((HardCodedImplementations.EfEntityObject_Customer)order.Customer.As<IPersistableObject>());
                    //customerRepo.ObjectSet.AddObject((HardCodedImplementations.EfEntityObject_Customer)order.Customer.As<IPersistableObject>());
