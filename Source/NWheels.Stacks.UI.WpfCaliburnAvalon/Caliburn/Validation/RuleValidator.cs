...
namespace NWheels.Stacks.UI.WpfCaliburnAvalon.Caliburn.Validation {
    ...
    public class RuleValidator : IValidator {
        private readonly IDictionary<string, IList<IValidationRule>> rules =
        ...
        public void AddRule(string propertyName, IValidationRule rule) {
            ...
            if (!rules.TryGetValue(propertyName, out current)) {
                current = new List<IValidationRule>();
                rules.Add(propertyName, current);
            }
