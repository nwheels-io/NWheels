...
namespace NWheels.Entities.Core
{
    public abstract class DataRepositoryBase : IApplicationDataRepository, IAccessControlContext
    {
        ...
        {
            if (!_entityRepositoryByContractType.ContainsKey(typeof(TEntityContract)))
            {
                _entityRepositoryByContractType.Add(typeof(TEntityContract), (IEntityRepository)repo);
                _genericCallbacksByContractType.Add(
                    typeof(TEntityContract),
                    callback => {
        ...
        public IQueryable<TEntity> AuthorizeQuery<TEntity>(IQueryable<TEntity> source)
        {
            ...
            var authorizedQuery = accessControl.AuthorizeQuery(this, source);

            var asQueryableOfTEntity = (authorizedQuery as IQueryable<TEntity>);

            if ( asQueryableOfTEntity != null )
            {
                return asQueryableOfTEntity;
            }
            else
            {
                return authorizedQuery.OfType<TEntity>();
            }
        }
        ...
        public abstract Type DomainContextContract { get; }
