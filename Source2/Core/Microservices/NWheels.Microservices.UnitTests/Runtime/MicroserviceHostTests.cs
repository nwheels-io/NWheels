...
namespace NWheels.Microservices.UnitTests.Runtime
{
    public class MicroserviceHostTests : TestBase.UnitTest
    {
        ...
        [Fact]
        public void Configure_FeatureLoaderInstantiationFailed_FaultedState()
        {
            //-- arrange
            ...
            var host = new MicroserviceHostBuilder("Test")
                .UseBootComponents(builder => {
                    builder.RegisterComponentType<TestModuleLoader>().ForService<IModuleLoader>();
                    builder.RegisterComponentInstance(bootLog);
                    FeatureWillDo<ApplicationFeatureOne>(builder, feature => throw rootCauseException);
                })
                .UseMicroserviceXml(XElement.Parse(@"
                    <microservice>
                        <framework-modules>
            ...
            host.CurrentState.Should().Be(MicroserviceState.Faulted);
            hostException.Reason.Should().Be(nameof(MicroserviceHostException.MicroserviceFaulted));
            hostException.InnermostException().Should().BeSameAs(rootCauseException);
            ...
            bootLog.Messages.Should().Equal(
                ...
                // step 1: feature loaders are instantiated
                "FrameworkFeatureOne:.ctor",        // Default from Kernel
                "ApplicationFeatureOne:.ctor",      // Default from AppM1 <-- will throw exception
                // state change -> Faulted
                "MicroserviceHost:CurrentStateChanged->Faulted"
                #endregion
            );
        ...
        [Fact]
        public void Configure_FeaturePhaseExtensionFailed_FaultedState()
        {
            //-- arrange
            ...
            var host = new MicroserviceHostBuilder("Test")
                ...
                "))
                .UseBootComponents(builder => {
                    FeatureWillDo<ApplicationFeatureTwo>(builder, feature => {
                        feature.PhaseExtension.OnBeforeContributeComponents += (cb) => {
            ...
            hostException.Reason.Should().Be(nameof(MicroserviceHostException.MicroserviceFaulted));
            hostException.InnerException.Should().BeOfType<MicroserviceHostException>().Which.Reason.Should().Be(nameof(MicroserviceHostException.FeatureLoaderFailed));
            featureException.FailedClass.Should().Be(typeof(ApplicationFeatureTwo));
            featureException.FailedPhase.Should().Be(nameof(IFeatureLoaderPhaseExtension.BeforeContributeComponents));
            featureException.InnerException.Should().BeSameAs(rootCauseException);
            ...
            bootLog.Messages.Should().Equal(
                ...
                // this only applies to feature loaders that have phase extension
                "FrameworkFeatureTwoPhaseExtension:BeforeContributeComponents",
                "ApplicationFeatureTwoPhaseExtension:BeforeContributeComponents", // <-- will throw exception 
                // state change -> Faulted
                "MicroserviceHost:CurrentStateChanged->Faulted"
                #endregion
            );
        ...
        [Fact]
        public void LoadPhase_ComponentConstructorFailed_FaultedState()
        {
            //-- arrange
            ...
            var host = (MicroserviceHost)new MicroserviceHostBuilder("Test")
                ...
                .UseBootComponents(builder => {
                    ...
                    // this will be invoked from ApplicationComponentOne constructor: 
                    ComponentWillDo<ApplicationComponentOne>(builder, component => throw rootCauseException); 
                })
                .BuildHost();
            ...
            hostException.Reason.Should().Be(nameof(MicroserviceHostException.MicroserviceFaulted));
            hostException.InnerException.Should().BeOfType<MicroserviceHostException>().Which.Reason.Should().Be(nameof(MicroserviceHostException.LifecycleComponentConstructorFailed));
            componentException.InnermostException().Should().BeSameAs(rootCauseException);
            ...
            runLog.Messages.Should().Equal(
                ...
                "FrameworkComponentOne:.ctor",
                "FrameworkComponentTwo:.ctor",
                "ApplicationComponentOne:.ctor", // <-- will throw exception
                // state changed -> Faulted
                "MicroserviceHost:CurrentStateChanged->Faulted"
                #endregion
            );
        ...
        [Fact]
        public void AttemptStopFromLoading_InvalidStateForStop_Throw()
        {
            //-- arrange

            MicroserviceHost host = null;
            host = (MicroserviceHost)new MicroserviceHostBuilder("Test")
                .UseBootComponents(builder => {
                    builder.RegisterComponentType<TestModuleLoader>().ForService<IModuleLoader>();
                    builder.RegisterComponentInstance(new TestLog());
                })
                .UseMicroserviceXml(XElement.Parse(@"
                    <microservice>
                        <application-modules>
                ...
                "))
                .UseBootComponents(builder => {
                    FeatureWillContributeComponentInstance<ApplicationFeatureOne, TestLog>(builder, new TestLog());
                    FeatureWillContributeLifecycleComponent<ApplicationFeatureOne, ApplicationComponentOne>(builder);
                    ComponentWillDo<ApplicationComponentOne>(builder, component => component.OnLoad += () => {
                        host.Stop(TimeSpan.FromSeconds(10)); // this is illegal and will result in MicroserviceHostException:InvalidStateForStop
                    });
                })
                .BuildHost();
            ...
            var componentException = hostException.InnerException.Should().BeOfType<MicroserviceHostException>().Which;
            componentException.Reason.Should().Be(nameof(MicroserviceHostException.LifecycleComponentFailed));
            componentException.FailedClass.Should().Be(typeof(ApplicationComponentOne));
            componentException.FailedPhase.Should().Be(nameof(ILifecycleComponent.Load));
            
            var rootCauseException = componentException.InnerException.Should().BeOfType<MicroserviceHostException>().Which;
            rootCauseException.Reason.Should().Be(nameof(MicroserviceHostException.InvalidStateForStop));
        ...
        public void RunDaemon_Success()
        {
            ...
            var microserviceActivated = new ManualResetEvent(false);
            var bootLog = new TestLog();
            var runLog = new TestLog();
            var host = (MicroserviceHost)new MicroserviceHostBuilder("Test")
            ...
            host.CurrentStateChanged += (sender, args) => {
                ...
                if (host.CurrentState == MicroserviceState.Active)
                {
                    microserviceActivated.Set();
                }
            };
            ...
            runLog.Add(null, "TEST-SIGNAL");
            cancellation.Cancel();
            daemonTask.Wait(TimeSpan.FromSeconds(10)).Should().BeTrue();
        ...
        public abstract class TestFeatureLoaderWithPhaseExtension : TestFeatureLoader, IFeatureLoaderWithPhaseExtension
        {
            ...
            IFeatureLoaderPhaseExtension IFeatureLoaderWithPhaseExtension.PhaseExtension => _phaseExtension;

            //-------------------------------------------------------------------------------------------------------------------------------------------------

            public TestFeatureLoaderPhaseExtension PhaseExtension => _phaseExtension;
        ...
        public class ApplicationFeatureTwo : TestFeatureLoaderWithPhaseExtension
        {
            ...
            internal class ApplicationFeatureTwoPhaseExtension : TestFeatureLoaderPhaseExtension
            {
