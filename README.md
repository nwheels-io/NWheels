...
NWheels supplies architectural recipes that tell the whole story of a typical enterprise application. This includes (but is not limited to): data persistence and caching, business logic in several architectural flavors, user interface on various platforms, B2B integrations, extensibility and customization, scalability and availability, testability, and DevOps/ALM toolchain.

### NWheels is Interface

Rather than being a monolith that attempts to solve every problem, NWheels only solves one problem, and it does it well. NWheels supplies consistent API and a set of core services to pluggable modules. It is then pluggable modules that define application capabilities, productivity frameworks for application development, and technology choices.

### NWheels is Generator

NWheels reduces amount of code that application developers have to write. It leaves the very minimum that declares and implements unique features of the application. 

One part of the "unwritten" code comes from pluggable modules, such as reusable domain models or adaptations to concrete technology stacks. 

The other part is a glue layer, which connects concrete application models with generic infrastructure mechanisms. The glue code is not unique, but rather derivative and mechanical. This part of code is generated by the platform.

### NWheels is Home

NWheels is aimed to become a place where collective expertise of enterprise application development is accumulated. Collaboration and expertise sharing is possible though development of highly adaptive and reusable NWheels modules, and the surrounding ecosystem of developer resources.
